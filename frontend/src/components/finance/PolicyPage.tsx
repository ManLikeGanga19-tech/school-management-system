      ...
      <AppShell title="Director" nav={nav} activeHref={activeHref}>
        <div className="flex min-h-[380px] items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-sm text-[var(--tenant-muted)]">Loading policy…</p>
          </div>
        </div>
      </AppShell>
    );
    ...
    <AppShell title="Director" nav={nav} activeHref={activeHref}>
      <div className="space-y-6">
        ...
        <div className="rounded-2xl border border-[var(--tenant-border)] bg-white shadow-sm">
          <div className="flex items-center gap-2 border-b border-[var(--tenant-border)] px-6 py-4">
            <ShieldCheck className="h-4 w-4 text-[var(--tenant-muted)]" />
            <h2 className="text-sm font-semibold text-[var(--tenant-ink)]">Enrollment Payment Rules</h2>
          </div>

          <div className="space-y-6 px-6 py-6">
            {/* Interview fee requirement */}
            <div className="flex items-start justify-between gap-6 rounded-xl border border-[var(--tenant-border)] p-4">
              <div>
                <p className="text-sm font-medium text-[var(--tenant-ink)]">
              ...
              <button
                ...
                className={`relative mt-0.5 h-6 w-11 flex-shrink-0 rounded-full transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--tenant-primary)] ${
                  policy.require_interview_fee_before_submit ? "bg-[var(--tenant-primary)]" : "bg-[var(--tenant-border)]"
                }`}
              >
                <span
            ...
            <div className="space-y-4 rounded-xl border border-[var(--tenant-border)] p-4">
              <div className="flex items-start justify-between gap-6">
                ...
                <button
                  ...
                  className={`relative mt-0.5 h-6 w-11 flex-shrink-0 rounded-full transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--tenant-primary)] ${
                    policy.allow_partial_enrollment ? "bg-[var(--tenant-primary)]" : "bg-[var(--tenant-border)]"
                  }`}
                >
                  <span
                    className={`absolute top-0.5 left-0.5 h-5 w-5 rounded-full bg-white shadow transition-transform ${
                ...
                <div className="grid grid-cols-2 gap-4 pt-2">
                  <div className="space-y-1.5">
                    <Label className="text-xs">
                      Minimum % to enroll{" "}
                      <span className="text-[var(--tenant-muted)]">(optional)</span>
                    </Label>
                    <div className="relative">
                      <Input
                        ...
                        className="pr-8"
                      />
                      <span className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-sm text-[var(--tenant-muted)]">
                        %
                      </span>
                    </div>
          ...
          <div className="flex justify-end border-t border-[var(--tenant-border)] px-6 py-4">
